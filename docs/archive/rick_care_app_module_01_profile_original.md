# Module 01 — Patient Profile & Clinical Context (Deep Spec v1.1)

**Status:** Phase A. Build-ready.

**Dependencies:** Technical foundation v1.0 (required — data model, auth, RLS patterns, UI components). Coordinator Agent Instructions v1.0 (required — integration gate reviews).

**Dependents:** Every other module. This is the foundational patient data layer.

**Related outline sections:** Outline v0.3 Module 1.

**Change log v1.0 → v1.1:** Added Section 14 (Multi-Agent Build Instructions) specifying how an Implementation Agent should work with the Coordinator Agent to build this module. Added Section 15 (Integration Checkpoints) specifying cross-module contracts Module 01 must honor.
...
### 4.2 Profile home (the dashboard)

This is the single most important screen in the module. It's the first thing users see after login (assuming they're on Rick's record, which most will be).

Layout — mobile portrait (375px):

```
┌─────────────────────────────────────┐
│  [Avatar]  Rick Whitman            │
│            78 y.o. • DOB 04/15/1948│
│            MRN: [household-local]   │
├─────────────────────────────────────┤
│  🔴 ACTIVE CONDITIONS               │
│  • Lumbar fusion L1-L5 + sacral    │
│    hardware (03/12/2026)            │
│  • Two small pulmonary emboli       │
│  • Pneumonia (active antibiotics)   │
...
│  [  Generate ER PDF  ]              │
├─────────────────────────────────────┤
│  BLT PRECAUTIONS: ACTIVE            │
│  Lifts: ~06/12/2026 (pending MD)   │
├─────────────────────────────────────┤
│  MPA: Inactive (Rick capacitated)  │
│  Holder: [Sister's name]            │
├─────────────────────────────────────┤
│  [ Demographics ] [ Surgeries ]    │
│  [ Diagnoses   ] [ Allergies  ]    │
│  [ Providers   ] [ Insurance  ]    │
│  [ Emergency   ] [ Advance    ]    │
│  [ Contacts    ] [ Directive  ]    │
...
Design principles on this screen:
- **Active conditions** are the very top, red because they're life-affecting. Allergies below in amber.
- **ER handoff is one tap away** — it's a primary action, not buried in a menu.
- **BLT status is surfaced** because it drives so many other decisions.
- **MPA status is visible and honest** — shows Rick's capacitated state explicitly, so sister can see at a glance that MPA is not active.
- **No "who entered this"** visible — that's the "facts, not blame" default.

Desktop layout: same data, two-column, with active conditions and emergency actions on the left, and a matrix of all sub-sections on the right.

### 4.3 Surgery detail screen

The surgery detail is particularly important because hardware inventory is what ER docs and imaging techs need.
...
│  BLT PRECAUTIONS                    │
│  Duration: 12 weeks                 │
│  Lift date: 06/04/2026              │
│  (pending surgeon clearance)        │
├─────────────────────────────────────┤
│  HARDWARE INVENTORY                 │
│                                     │
│  Pedicle screws (10 × titanium)    │
│  Levels: L1, L2, L3, L4, L5        │
│  Medtronic CD Horizon               │
│  MRI: conditional 1.5T/3T           │
│                                     │
│  Rods (2 × titanium, 12 cm)         │
│  Medtronic                          │
│                                     │
│  Sacral instrumentation             │
│  Stryker [model]                    │
│  MRI: conditional 1.5T              │
│                                     │
│  Hip prosthesis, L (2019)           │
│  Zimmer [model], cobalt-chromium    │
│                                     │
│  Hip prosthesis, R (2019)           │
│  Zimmer [model], cobalt-chromium    │
│                                     │
│  Notes: Sacral hardware tied to    │
│  bilateral hip prostheses for       │
│  stability                          │
├─────────────────────────────────────┤
│  DOCUMENTS                          │
│  • Operative report (03/12/2026)   │
│  • Discharge summary (03/18/2026)  │
│  • Pre-op imaging (03/10/2026)     │
│  [ + Add document ]                 │
├─────────────────────────────────────┤
│  [ Edit surgery ] [ Edit hardware ]│
└─────────────────────────────────────┘
```

Design principles:
- Hardware renders as a readable summary, not raw JSONB. The JSONB structure enforces consistency across data entry; the display layer renders it for humans.
- MRI compatibility is visible per implant — this is the single most common "why do you need this info" ask for imaging techs.
- Documents link to the Document Vault (Module 13) but display summary metadata here.

### 4.4 Allergies screen
...
│  Penicillin                         │
│  → Anaphylaxis                      │
│  Verified: Dr. [X], 02/2018         │
├─────────────────────────────────────┤
│  MODERATE                           │
│  Sulfa drugs                        │
│  → Rash                             │
│  Verified: [name], 06/2020          │
├─────────────────────────────────────┤
│  MILD                               │
│  Latex                              │
│  → Contact dermatitis               │
...
Design principles:
- Severity determines order: life-threatening always at the top, in red.
- Life-threatening allergies get an icon that never disappears even when the section is collapsed.
- "Verified" field shows source — reduces the "where did this come from" question. Critical for Sister's reconciliation work.

### 4.5 MPA status screen
...
├─────────────────────────────────────┤
│  TO ACTIVATE                        │
│  MPA becomes active when a          │
│  physician documents Rick as        │
│  incapacitated. Upload a            │
│  capacity determination to          │
│  activate.                          │
│                                     │
│  [ Upload capacity determination ]  │
└─────────────────────────────────────┘
```

Design principles:
- **Default state is visible and reassuring.** "Inactive" + "Rick is the decision-maker" is a green status, not a warning.
- **The activation path is transparent.** Sister can see exactly what's required — a clinician-signed document — so the process is not mysterious or gameable.
- **History is preserved.** Every activation and deactivation is logged here (empty for now, populated over time).

### 4.6 ER handoff generator

Single-tap generation is the goal. But a review step before sending/sharing is important — EMTs have handed patients the wrong ER summary before.
...
Design principles:
- **Review-first.** Users see what will be included before generating.
- **Single action.** Generate, then share. No multi-step wizard.
- **Default-safe inclusions.** Insurance excluded by default (not needed for clinical care and a privacy concern if the PDF is lost).
- **Share via the OS native sheet** on mobile (which gives AirDrop, email, messages, print, etc.). Fewer abstractions = more reliable.

### 4.7 Reusable components (shared with other modules)

- `ActiveConditionsCard` — used on dashboard; displays current diagnoses with severity indicators.
- `AllergyBadge` — compact representation of an allergy; used in ER handoff, medication add flow, anywhere relevant.
- `HardwareDisplay` — renders JSONB hardware_inventory as human-readable list.
- `MPAStatusIndicator` — small indicator of MPA state; used in app header.
- `ERHandoffButton` — triggers the generator modal; used in header, dashboard, fall event workflow.
- `ProviderContactCard` — used here and in Modules 08, 15.
- `DocumentThumbnail` — used here, in 13, in 15.
...
**Actors:** Usually Ben (setting it up for the family) or Linda.

**Minimum viable profile** (cannot proceed past onboarding without these):
- Household name
- Primary patient: legal name, DOB, biological sex
- At least one emergency contact with phone
- Surgeon name + phone OR a primary diagnosis and one provider
- One allergy OR explicit "no known allergies" acknowledgment

**Progressive enhancement** (can skip during onboarding, prompted later):
- Detailed surgical record (can be added after discharge summary is in hand)
- Hardware inventory (can be extracted from op report later)
- Comorbidities (can be reconciled during diagnosis reconciliation workflow)
- Insurance (needed for Module 08 but not for Phase A basics)
- MPA holder info (prompted at end of onboarding)

**Flow:**

1. Welcome screen, explains what the app does and who uses it.
2. Household name + timezone.
3. Primary patient basics: legal name, preferred name, DOB, biological sex, height, weight.
4. "What happened surgically?" — free-text OR structured (depending on user confidence).
5. Allergies quick-entry: "No known allergies" OR start adding.
6. Emergency contacts: who to call if something happens. At least one required.
7. Who else uses the app? Invite co-users (Linda, Sister, Ben). Each invite sends an email link to create an account with pre-assigned role.
8. MPA holder info (recommended but skippable).
9. Done → lands on Profile home screen with "Next steps" prompts to fill in the rest.

**Validation:**
- Household name: 1–100 characters.
- Patient name: 1–100 characters.
- DOB: must be in the past, must yield reasonable age (0–120).
- Phone numbers: validated format.
- Email invitations: valid email format; cannot invite the same email twice.

**Edge cases:**
- User backs out mid-flow → draft is saved to IndexedDB, resumable.
- User closes browser → same.
- Duplicate email invite → error message, offer to resend.
...
1. **Schema migrations first.** All tables, indexes, enums, RLS policies. One migration per table for reviewability.
...
4. **Data access layer** — TanStack Query hooks for CRUD operations on each entity.
5. **Shared components** — `ActiveConditionsCard`, `AllergyBadge`, `HardwareDisplay`, `MPAStatusIndicator`, `ProviderContactCard`.
6. **Screens in order:** Profile home → Demographics editor → Surgery list → Surgery detail → Hardware editor → Allergies → Providers → Insurance → Emergency contacts → Advance directive → MPA → Fall history.
7. **ER handoff** last, because it aggregates everything.
8. **Workflows** (onboarding, MPA activation, diagnosis reconciliation) after screens exist.
...
|---|---|---|---|
| 1. Schema migrations | SQL migrations for all tables in §3, one file per table | Naming conventions, RLS enabled on every table, FK integrity, audit triggers | No (CA-only if clean) |
| 2. Types & schemas | TS types (via `supabase gen types`), Zod schemas for all forms and API boundaries | Type fidelity to schema, Zod validation matches §7 validation rules | No |
| 3. Data access layer | TanStack Query hooks for CRUD on each entity, with optimistic updates and cache invalidation | Query keys consistent, invalidation correct, RLS-aware error handling | No |
| 4. Shared components | `ActiveConditionsCard`, `AllergyBadge`, `HardwareDisplay`, `MPAStatusIndicator`, `ProviderContactCard`, `TimestampedEntry`, `ERHandoffButton` | Reusability (components must be usable by future modules), accessibility (WCAG AA), Storybook coverage | No |
| 5. Screens | All screens in §4.1, responsive, offline-aware | Screen composition uses shared components, no duplicated logic, matches wireframes | **Yes (design prototype sign-off)** |
| 6. Workflows | Onboarding, MPA activation, diagnosis reconciliation, ER handoff generation | Workflow logic matches §5 step-by-step, edge cases from §8 handled, audit log entries correct | **Yes** |
| 7. Tests | All acceptance criteria from §10 covered by E2E tests; unit + integration tests for all logic | AC-to-test mapping, test independence, RLS policy tests pass for all role combinations | **Yes (final sign-off)** |
...
Module 01 screens require a design package before Phase 5 (Screens). The design package includes:

- Design Principles document (colors, typography, spacing, component patterns)
- Component library prototype (interactive artifact showing each shared component in each state)
- Screen wireframes or mockups for each screen in §4.1
- Module 01 clickable prototype

These are produced in a separate design workflow (see Design Workflow Guide) and approved by the human before the IA begins Phase 5.

If the design package is not yet available, the IA can proceed through Phases 1–4 (which are presentation-agnostic) and halt at the Phase 4/5 gate until design is approved.
